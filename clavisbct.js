...
// https dal 27 settembre 2017
bctHostPort='https://clavisbct.comperio.it'; // prod
...
function check_item_record(item_id) {
    ...
    var t=jQuery('#ctl0_Main_UpdateData_CreatedBy').parent().next().next();
    var date_updated=jQuery('span.viewlabel2small',t).text().trim();
    var item={
	...
	"item_media": jQuery('#ctl0_Main_ItemView_ItemMedia').text(),
	"loan_status": jQuery('#ctl0_Main_ItemView_LoanStatusString').text(),
	"opac_visible": document.getElementById('ctl0_Main_ItemView_OpacVisible').checked,
...
function setOpacLink(manifestation_id) {
    ...
    jQuery('#ctl0_TabBar_OpacHyperLink').prop('href', 'https://bct.comperio.it/opac/detail/view/sbct:catalog:' + manifestation_id).css("color", "green");
}
...
function Circulation_ManageRequests() {
    init_clavisbct();
    var item_ids='',
	url=bctHostPort + '/clavis_items?';

    // Devo estrarre id esemplare da (esempio):
    // http://sbct.comperio.it/index.php?page=Catalog.ItemViewPage&amp;id=450841

    var rows=jQuery('tr','#ctl0_Main_Requests_Grid');

    // seleziono solo le righe con la bandierina
    rows=rows.has('img[src="themes/Default/icons/flag_red.png"]');
    jQuery(rows).filter(function(index) {
	var r=jQuery('a[href*="ItemViewPage"]',this);
	item_ids += r[0].href.split('=').reverse()[0] + '+';
    });
    ...
    link=jQuery('<a/>', {
	href: url,
	title: 'Stampa lista in ordine di collocazione',
	target: '_blank',
	text: 'Stampa'
    });
    ...
    link_in_gestione=jQuery('<a/>', {
	href: url,
	title: 'Stampa lista in ordine di collocazione',
	target: '_blank',
	text: '[Stampa]'
    });
    jQuery('#ctl0_Main_Requests_ctl9').after(link);
...
function Circulation_ReservationList() {
	...
	manifestation_ids='',
	item_ids='';
    var rows=jQuery('tr','#ctl0_Main_Reservation_Grid');

    // seleziono solo le righe gestite dal librarian loggato al momento
    librarian_id=getOperatorId();
    // librarian_id=588;

    librarian_url="index.php?page=Library.LibrarianViewPage&id=" + librarian_id;
    
    rows=rows.has("a[href='" + librarian_url + "']");

    jQuery('a[href*="Catalog.Record"]',rows).filter(function(index,element) {
	manifestation_ids += element.href.split('=').reverse()[0] + '+';
    });
    jQuery('a[href*="Catalog.ItemViewPage"]',rows).filter(function(index,element) {
	item_ids += element.href.split('=').reverse()[0] + '+';
    });
    library_id=jQuery('#ctl0_Footer_ActualLibraryLink').attr('href').split('=').reverse()[0];
    url += 'library_id=' + library_id + '&amp;pdf_template=rawlist&amp;per_page=999999';
    url += '&amp;manifestation_ids=' + manifestation_ids;
    url += '&amp;item_ids=' + item_ids;
    url=encodeURI(url);
    var link=jQuery('<a/>', {
	href: url,
	title: 'Stampa lista esemplari disponibili in ordine di collocazione',
	target: '_blank',
...
function AuthorityList() {
	...
	document.location.href='https://sbct.comperio.it/index.php?page=Catalog.AuthorityViewPage&id=' + this.value;
    });
}
...
function AuthorityViewPage() {
    ...
    if((jQuery('em','#ctl0_Main_AuthViewer_AuthViewer').first().text().trim().substr(0,8)=='Soggetto') || (jQuery('em','#ctl0_Main_AuthViewer_AuthViewer').first().text().substr(0,8)=='Argoment')) {
	var authority_id=document.location.href.split("=").reverse()[0];
	var target=jQuery('#ctl0_Main_AuthViewer_AuthViewer');
	var url='https://clavisbct.comperio.it/subjects/0?embedded=true&amp;clavis_authority_id=' + authority_id;
	jQuery('<iframe/>', {
	    src: url,
	    width: target.width()-24 + 'px',
	    height: '1200px'
...
function BctOpacDetail(manifestation_id) {
    jQuery('#navigation li:last').after('<li><a href="https://sbct.comperio.it/index.php?page=Catalog.Record&manifestationId=' + manifestation_id + '">Vedi in Clavis</a></li>');
}
...
//    }
    ...
    if (document.location.href.match('Circulation.ManageRequests')) {
	return Circulation_ManageRequests();
    }
	...
	// return BctOpacDetail(manifestation_id);  // eliminato 27 settembre 2017
    }
